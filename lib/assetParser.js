...
LENGTH & DEPTH — this matters: a VC reads these as real answers, not labels.
Short fields — keep concise:
  company_name: the name only
  stage: the stage label only (e.g. "Pre-seed")
  one_liner: a single sentence
  raise_amount_hint: the amount / instrument phrase
  founder_names: names only
  governing_principle: one clear sentence
  readiness_verdict: the verdict (e.g. "READY FOR …") plus one sentence
  next_move: one or two sentences naming the specific action
Narrative fields — write 2-4 COMPLETE sentences that actually explain the answer,
synthesized from across the document. Never return a bare label or a few words:
  problem_statement, solution_description, competitive_landscape, target_customer,
  tam_description, business_model, go_to_market, key_metrics, why_unique, why_now,
  positioning_statement, use_of_funds

Ground every sentence in the document. Never invent facts, numbers, or claims. If the
document only supports a short answer, give the short answer rather than padding.
Confidence reflects how well the document supports the CONTENT, not its length — a
3-sentence answer whose every claim is supported is still high confidence.
